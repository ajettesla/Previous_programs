...
int Np = 1797;;		// number of entries in the data
...
if(strategy == 1){
    while(1){
      int csocketfd = accept(socket,(struct sockaddr*)&clientaddress,&addrlen);
      ...
      while(1){
        ...
        if(strncmp(buffer,terminatebuf,8) == 0){removeclientformarray(client);close(csocketfd);break;}
        else{
          ...
          int sent_recv_bytes = send(csocketfd, &filesize, sizeof(filesize),0);
          if(sent_recv_bytes < 0){removeclientformarray(client);close(csocketfd);perror("error with recv function 2");exit(1);}

          while(1){
                  sent_recv_bytes = fread(sendbuffer, 1, sizeof(sendbuffer), file);
                  if(sent_recv_bytes < 0){
                    break;
                  }
                  if(sent_recv_bytes == 0){break;}
                  sent_recv_bytes = send(csocketfd, sendbuffer, sent_recv_bytes, 0);
                  if(sent_recv_bytes < 0){close(csocketfd);removeclientformarray(client);perror("error with send function 3");break;}}
          ...
          int sent_recv_bytes = send(csocketfd, buffer, sizeof(buffer),0);
          if(sent_recv_bytes < 0){removeclientformarray(client);close(csocketfd);perror("error with sent function 4");exit(1);}
        }
        }
      }
    ...
    while(1){
      ...
      if(FD_ISSET(socket, &readfds)){
        int comsocket = accept(socket, (struct sockaddr*)&clientaddress,&addrlen);
        ...
        for(int i=0; i< getmax() + 1; i++){
          ...
          if(FD_ISSET(clientFD[i], &readfds)){
            for(int j=0; j< getmax(); j++){if(clientarray[j]->clientsocket == clientFD[i]){client = clientarray[j];socket = clientFD[i];break;}}}}}
          char buffer[100];
          memset(buffer, 0, sizeof(buffer));
          int sent_recv_bytes = recv(socket,buffer, sizeof(buffer),0);
          if(sent_recv_bytes < 0){removeclientformarray(client);removeclientfd(socket);close(socket);perror("error with recv function");exit(1);}
          buffer[sent_recv_bytes] = '\0';
          char terminatebuf[9] = "TERMINATE";
          //if terminate message is received than
          if(strncmp(buffer,terminatebuf,8) == 0){removeclientformarray(client);removeclientfd(socket);close(socket);}
         ...
         if(client->succesfulcal == 1){
          ...
          char sendbuffer[1024];
          int sent_recv_bytes = send(socket, &filesize, sizeof(filesize),0);
          if(sent_recv_bytes < 0){removeclientformarray(client);removeclientfd(socket);close(socket);perror("error with recv function");exit(1);}
          ...
          while(1){
                  ...
                  if(sent_recv_bytes == 0){break;}
                  sent_recv_bytes = send(socket, sendbuffer, sent_recv_bytes, 0);
                  if(sent_recv_bytes < 0){close(socket);perror("error with send function");break;}}
          ...
          printf("unable to make calculations\n");
          char buffer[] = "fail to calcutate results\n";
          int sent_recv_bytes = send(socket, buffer, sizeof(buffer),0);
          if(sent_recv_bytes < 0){removeclientformarray(client);removeclientfd(socket);close(socket);perror("error with sent function");exit(1);}
        }
...
void* callBackfun(void* buf){
    ...
    if(strcmp(substring,"mati")==0){
      ...
      startcalulation(data->filename, charcount, com);
