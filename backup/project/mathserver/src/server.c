    ...
    while(1){
      ...
      while(1){
        ...
        if(strncmp(buffer,terminatebuf,8) == 0){removeclientformarray(client);close(csocketfd);exit(0);}
          ...
          while(1){if(client->succesfulcal !=0){break;}}
          }
          ...
          while(1){
                  ...
                  sent_recv_bytes = send(csocketfd, sendbuffer, sent_recv, 0);
    ...
    while(1){
      ...
      if(FD_ISSET(socket, &readfds)){
        ...
        for(int i=0; i< getmax() + 1; i++){
          ...
          if(sent_recv_bytes < 0){removeclientformarray(client);close(socket);perror("error with recv function 2");exit(1);}
          while(1){
                  memset(sendbuffer, 0, sizeof(sendbuffer));
                  int sent_recv = fread(sendbuffer, 1, sizeof(sendbuffer), file);
                  if(sent_recv < 0){break;}
                  if(sent_recv_bytes == 0){break;}
                  sent_recv_bytes = send(socket, sendbuffer, sent_recv, 0);
                  if(sent_recv_bytes < 0){close(socket);removeclientformarray(client);removeclientfd(socket);close(socket);perror("error with send function 3");break;}}
